...
morePostsBtn.onclick = function () {
  fetch(
    `http://localhost:3000/posts?_pos=${pos}&_start=${contadorPosts}&_limit=15`
  )
    .then((response) => response.json())
    .then((data) => listPosts(data));
  contadorPosts += 15;
...
function listPost(post, cont, div) {
  ...
  if (cont < 99) {
    div.className = "row";
    div.innerHTML += `<div class="col-lg-4 col-md-6 col-sm-12">
...
document.addEventListener("click", function (e) {
  if (e.target.matches("[data-comments]"))
    viewComments(e.target.dataset.comments);
});
...
confirmSuprimir.onclick = () => {
  fetch(`http://localhost:3000/posts/${dataId}`, { method: "DELETE" }).then(
